Skill.createBy("spirit_of_fight:mace.block", "spirit_of_fight:block", builder => {
    builder.addEntityAnimatableCondition()
    builder.acceptConfig(config => {
        config.set("anim_path", "spirit_of_fight:spirit_of_fight/animations/player/fight_skill/fightskill_mace/mace.guard")
    ...
    builder.accept(skill => {
        ...
        skill.onBlockHurt((event, hitPos) => {
            level.playSound(entity.getOnPos().above(), "spirit_of_fight:heavy_block", "players")
            SOFParticlePresets.summonQuadraticParticle(level, hitPos, 15, 'minecraft:crit')
        })

        skill.onPrecisionBlock((event, hitPos) => {
            level.playSound(entity.getOnPos().above(), "spirit_of_fight:heavy_parry_1", "players")
