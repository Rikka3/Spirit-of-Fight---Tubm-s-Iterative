Skill.create("spirit_of_fight:gloves_combo_0", builder => {
    builder.accept(skill => {
        ...
        const anim = animatable.createAnimation('minecraft:player', 'baimei:combo_0')
        ...
        skill.onActive(() => {
            ...
            if (animTime >= 0.1 && animTime <= 0.2) {
                entity.move(SpMath.vec3(0.0, entity.getDeltaMovement().y, 0.35), false)
            } else if (animTime >= 0.4 && animTime <= 0.5) {
                entity.move(SpMath.vec3(0.0, entity.getDeltaMovement().y, 0.25), false)
            }

            if (animTime >= 0.15 && animTime <= 0.2) {
