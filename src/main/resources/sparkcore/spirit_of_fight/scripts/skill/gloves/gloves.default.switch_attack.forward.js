Skill.create("spirit_of_fight:gloves.default.switch_attack.forward", builder => {
    ...
    builder.acceptConfig(config => {
        ...
        config.set("target_knockback_strength", 0.65)
    ...
    builder.accept(skill => {
        ...
        attackBody.onAttackCollide('attack', {
            ...
            doAttack: (attacker, target, o1, o2, manifoldId, attackSystem) => {
                entity.sofCommonAttack(target, "heavy_stab", 25, 25)
            },
        ...
        anim.registerKeyframeRanges("attack", [[0.2, 0.4]], (attackKF, index) => {
            attackKF.onEnter(() => {
                attackBody.setCollideWithGroups(1)
                entity.move([0.0, entity.getDeltaMovement().y, 0.85], false)
                level.playSound(entity.getOnPos().above(), "minecraft:entity.player.attack.weak", "players")
                entity.setCameraLock(true)
            })
            attackKF.onInside(() => {
                animatable.summonTrail(trailMesh, "rightLeg", [0.0, -0.5, 0.0], [0.0, -0.7, 0.0])
            })
            attackKF.onExit(() => {
