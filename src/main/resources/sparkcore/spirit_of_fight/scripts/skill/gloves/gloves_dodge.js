Skill.createBy("spirit_of_fight:gloves_dodge", "spirit_of_fight:dodge", builder => {
    builder.acceptConfig(config => {
        config.put('anim_name', 'baimei:dodge')
        config.put('enable_perfect_dodge', true)
        config.put('max_perfect_dodge_times', 5)
    })
    builder.accept(skill => {
        ...
        skill.onPerfectDodge((event, times) => {
            if (times == 0) level.playSound(entity.getOnPos().above(), "spirit_of_fight:perfect_dodge", "players")
