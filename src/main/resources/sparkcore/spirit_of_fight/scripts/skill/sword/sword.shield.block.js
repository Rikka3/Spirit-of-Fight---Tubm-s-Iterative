Skill.createBy("spirit_of_fight:sword.shield.block", "spirit_of_fight:block", builder => {
    builder.addEntityAnimatableCondition()
    builder.acceptConfig(config => {
        config.set("anim_path", "spirit_of_fight:spirit_of_fight/animations/player/fight_skill/sword/sword.skills.common/sword.shield_block")
    ...
    builder.accept(skill => {
        ...
        config.set("block_body", PhysicsHelper.createCollisionBoxBoundToBone(animatable, 'leftItem', [0.5, 1.0, 2.0], [-0.25, 0.25, 0.0], body => { body.setCollideWithGroups(1) }))

        skill.onBlockHurt((event, hitPos) => {
            level.playSound(entity.getOnPos().above(), "minecraft:item.shield.block", "players")
