Skill.create("spirit_of_fight:spear.special.combo_2", builder => {
    ...
    builder.accept(skill => {
        ...
        anim.registerKeyframeRanges("attack", [[0.3, 0.75], [0.8, 1.0]], (kf, index) => {
            kf.onEnter(() => {
                attackBody.setCollideWithGroups(1)
                if (index == 1) entity.move([0.0, entity.getDeltaMovement().y, 0.5], false)
                entity.setCameraLock(true)
                level.playSound(entity.getOnPos().above(), "spirit_of_fight:sharp_wield_1", "players", 1, 1)
            })
            kf.onInside(() => {
                if (index == 0) entity.move([0.0, entity.getDeltaMovement().y, 0.25], false)
