Skill.create("spirit_of_fight:spear.default.combo_3", builder => {
    ...
    builder.accept(skill => {
        ...
        anim.registerKeyframeRanges("attack", [[0.2, 0.4], [0.4, 0.6], [0.9, 1.1]], (kf, index) => {
            kf.onEnter(() => {
                if (index == 2) skill.getConfig().set("damage_multiplier", 1.5)
                attackBody.setCollideWithGroups(1)
                entity.setCameraLock(true)
                level.playSound(entity.getOnPos().above(), "spirit_of_fight:sharp_wield_1", "players", 1, 1.1 - 0.1 * index)
            })
            kf.onInside(() => {
