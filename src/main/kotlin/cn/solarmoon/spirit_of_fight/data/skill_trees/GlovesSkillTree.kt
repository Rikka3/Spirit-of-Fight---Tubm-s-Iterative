...
object GlovesSkillTree {
    ...
    fun register(context: BootstrapContext<SkillTree>) {
        context.register(GLOVES_SPECIAL_ATTACK,
            SkillTree(
                ...
                listOf(
                    cn.solarmoon.spirit_of_fight.skill.tree.node.FightSpiritConsumeNode(
                        ...
                        SOFSkillTypes.GLOVES_SPECIAL_ATTACK,
                        SOFPreInputs.ATTACK
                    )
                ),
                4
            ...
            SkillTree(
                ...
                listOf(
                    ConditionNode(
                        ...
                        listOf(
                            CommonNode(
                                listOf(KeyInputCondition(mapOf(SOFKeyMappings.SWITCH_POSTURE.get().name to KeyEvent.PRESS_ONCE)), AnyCondition(ConcurrentLinkedQueue(listOf(InputDirectionCondition(MoveDirection.FORWARD), InputDirectionCondition(null))))),
                                SOFSkillTypes.GLOVES_DEFAULT_SWITCH_ATTACK_FORWARD,
                                SOFPreInputs.SWITCH_POSTURE,
                                cooldown = 40
                            ),
                            CommonNode(
                                listOf(KeyInputCondition(mapOf(SOFKeyMappings.SWITCH_POSTURE.get().name to KeyEvent.PRESS_ONCE)), InputDirectionCondition(MoveDirection.LEFT)),
                                SOFSkillTypes.GLOVES_DEFAULT_SWITCH_ATTACK_LEFT,
                                SOFPreInputs.SWITCH_POSTURE,
                                cooldown = 40
                            ),
                            CommonNode(
                                listOf(KeyInputCondition(mapOf(SOFKeyMappings.SWITCH_POSTURE.get().name to KeyEvent.PRESS_ONCE)), InputDirectionCondition(MoveDirection.RIGHT)),
                                SOFSkillTypes.GLOVES_DEFAULT_SWITCH_ATTACK_RIGHT,
                                SOFPreInputs.SWITCH_POSTURE,
                                cooldown = 40
                            ),
                            CommonNode(
                                listOf(KeyInputCondition(mapOf(SOFKeyMappings.SWITCH_POSTURE.get().name to KeyEvent.PRESS_ONCE)), InputDirectionCondition(MoveDirection.BACKWARD)),
